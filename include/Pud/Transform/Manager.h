#ifndef PUD_TRANSFORM_MANAGER_H
#define PUD_TRANSFORM_MANAGER_H
...
class PassManager {
 private:
  ...
  class KeyManager {
   ...
   public:
    ...
    auto get_unique_key(const std::string& key) -> std::string;
  };
  ...
  struct PassMetadata {
    ...
    auto operator=(PassMetadata&&) -> PassMetadata& = default;
  };
  ...
  struct AnalysisMetadata {
    ...
    auto operator=(AnalysisMetadata&&) -> AnalysisMetadata& = default;
  };
 ...
 public:
  ...
  explicit PassManager(Init init, std::vector<std::string> disabled = {},
                       bool py_numerics = false, bool py_extension = false)
      : km(),
        passes(),
        analyses(),
        execution_order(),
        ...
        py_numerics(py_numerics),
        py_extension(py_extension) {
    register_standard_passes(init);
  }

  explicit PassManager(bool debug = false,
                       std::vector<std::string> disabled = {},
                       bool py_numerics = false, bool py_extension = false)
      : PassManager(debug ? Init::DEBUG : Init::RELEASE, std::move(disabled),
                    py_numerics, py_extension) {}

  /// Checks if the given pass is included in this manager.
  /// @param key the pass key
  /// @return true if manager has the given pass
  auto has_pass(const std::string& key) -> bool {
  ...
  auto has_analysis(const std::string& key) -> bool {
  ...
  auto register_pass(std::unique_ptr<Pass> pass,
                     const std::string& insert_before = "",
                     std::vector<std::string> reqs = {},
                     std::vector<std::string> invalidates = {}) -> std::string;
  ...
  auto register_analysis(std::unique_ptr<Analyze::Analysis> analysis,
                         std::vector<std::string> reqs = {}) -> std::string;
  ...
  /// @return the result
  auto get_analysis_result(const std::string& key) -> Analyze::Result* {
 ...
 private:
  void run_pass(Module* module, const std::string& name);
  void register_standard_passes(Init init);
  void run_analysis(Module* module, const std::string& name);
  void invalidate(const std::string& key);
};

}  // namespace Pud::IR::Transform

#endif  // PUD_TRANSFORM_MANAGER_H